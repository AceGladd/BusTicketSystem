...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_coupon'])) {
    ...
    // company_id can be NULL for global coupons
    if (empty($company_id)) {
        $company_id = null;
    }
    ...
    if (empty($code) || empty($discount) || empty($usage_limit) || empty($expire_date)) {
        ...
        if ($stmt->fetch()) {
            ...
            if (empty($coupon_id)) {
                ...
                if ($stmt->execute([$coupon_id, $code, $discount, $company_id, $usage_limit, $expire_date])) {
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Firma (Boş bırakılırsa genel kupon)</label>
                        <select class="form-select" name="company_id">
                            <option value="">Tüm Firmalar (Genel)</option>
                            <?php foreach ($companies as $company): ?>
                                <option value="<?php echo $company['id']; ?>"
                                    <?php echo (isset($editCoupon['company_id']) && $editCoupon['company_id'] === $company['id']) ? 'selected' : ''; ?>>
                    ...
                    <?php if ($editCoupon): ?>
                        <a href="/admin/coupons.php" class="btn btn-secondary w-100 mt-2">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="/admin/coupons.php?edit=<?php echo $coupon['id']; ?>"
